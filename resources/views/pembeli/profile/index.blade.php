...
<body style="background-image: url('/assets/images/tamu/bg.png'); background-size: cover;" class="bg-[#618264] h-screen">
    ...
    <div class="ms-3 flex items-center pt-3">
        <a class="font-semibold text-2xl text-white flex items-center ml-3" href="{{ route('dashboard') }}"><i class="fa-solid fa-arrow-left mr-3"></i>Kembali</a>
    ...
    <div class="container p-0 mt-4 mb-8">
        <div class="grid grid-rows-3 grid-flow-col gap-4">
            <div class="row-span-3">
                @if($user->foto != null)
                <img src="{{ asset('/storage/assets/images/profile/' . $user->foto) }}" alt="Foto Profil" class="w-60 rounded-lg mt-5 ml-28" />
                @else
            ...
            <div class="row-span-3 col-span-2">
                <div class="grid grid-cols-2 gap-4 -ml-44 mt-4">
                    <div class="col-span-1">
                        ...
                        <input type="text" class="form-control w-11/12 rounded-lg border-slate-950 mt-1" name="nama_pengguna" readonly value="{{ $user->nama }}" />
                    </div>
                    <div class="col-span-1 -ml-3">
                        <label class="form-label text-lg font-semibold" style="color: #03045e">Nomor HP</label>
                        <input type="number" class="form-control w-8/12 rounded-lg border-slate-950 mt-1" name="nomor_hp" readonly value="{{ $user->nomor_hp }}" />
                    </div>

                    <div class="col-span-2">
                        <label class="form-label text-lg font-semibold" style="color: #03045e">Alamat</label>
                        <textarea class="form-control w-10/12 rounded-lg border-slate-950 mt-1" rows="3" name="alamat" readonly>{{ $user->alamat }}</textarea>
                    </div>
                    <div class="col-span-2">
                        <label class="form-label text-lg font-semibold" style="color: #03045e">Email</label>
                        <input type="text" class="form-control w-10/12 rounded-lg border-slate-950 mt-1" name="email" readonly value="{{ $user->email }}" />
                    </div>
                </div>
            </div>
        </div>
        <div class="grid grid-cols-4 gap-4 mt-32">
            <div class="">
                <a class="btn fs-4 font-bold w-full ml-20 h-16 text-2xl" style="border: 3px solid #0077b6; color: #0077b6" href="{{ route('profile.edit') }}" role="button"><i class="fa-solid fa-pen"></i>Ubah
                </a>
            </div>
        </div>
    </div>
